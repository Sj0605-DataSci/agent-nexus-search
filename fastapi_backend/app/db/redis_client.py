...
def cached(ttl: int = 300, key_prefix: str = "func"):
    """
    Decorator to cache function results using RedisClient.
    ...
    def decorator(func: Callable):
        if asyncio.iscoroutinefunction(func):
            ...
            async def async_wrapper(*args, **kwargs):
                ...
                # Extract identifiers
                if hasattr(state, "get"):  # dict-like
                    user_id = state.get(
                        "user_id", state.get("agent_config", {}).get("user_id", "global")
                    )
                    query = state.get("user_query", "")
                elif hasattr(state, "user_id") or hasattr(state, "agent_config"):
                    user_id = getattr(state, "user_id", None) or getattr(
                        getattr(state, "agent_config", {}), "get", lambda x, y=None: y
                    )("user_id", "global")
                    messages = getattr(state, "messages", [])
                    if messages:
                        query = getattr(messages[0], "content", "")

                # Build cache key
                sanitized_query = str(query).replace(" ", "_").lower()
                cache_key = f"{key_prefix}:{user_id}:{func.__name__}:{sanitized_query}"
