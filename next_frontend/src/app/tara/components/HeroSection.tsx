...
import { ArrowRight, Shield, CheckCircle, Zap, TrendingUp, FileText, PieChart, MessageCircle } from "lucide-react";
...
const FloatingIcon = ({ icon, className }: { icon: React.ReactNode, className: string }) => (
  <div className={`absolute ${className} w-16 h-16 bg-white/80 backdrop-blur-sm rounded-2xl flex items-center justify-center shadow-lg border border-white/50`}>
    {icon}
  </div>
);
...
const HeroSection = () => {
  const icons = [
    { icon: <TrendingUp className="w-8 h-8 text-blue-500" />, className: "top-[15%] left-[10%] animate-float" },
    { icon: <FileText className="w-8 h-8 text-green-500" />, className: "top-[30%] right-[12%] animate-float-delay-1" },
    { icon: <PieChart className="w-8 h-8 text-indigo-500" />, className: "bottom-[20%] left-[18%] animate-float-delay-2" },
    { icon: <MessageCircle className="w-8 h-8 text-purple-500" />, className: "bottom-[15%] right-[8%] animate-float-delay-3" },
  ];

  return (
    <section
      id="hero"
      className="bg-white text-gray-800 py-24 lg:py-32 relative overflow-hidden"
    >
