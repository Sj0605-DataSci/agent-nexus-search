...
export const isEnhancedProfileData = (content: string): boolean => {
  ...
  try {
    ...
    const hasProfileStructure = /first_name.*last_name.*yes_score.*maybe_score.*no_score/.test(content);
    const hasJsonArrayStructure = (content.trim().startsWith('[') && content.trim().endsWith(']')) ||
...
export const sanitizeEnhancedProfile = (profile: any): EnhancedProfile => {
  return {
    ...
    yes_score: {
      ...
      matching_traits: Array.isArray(profile.yes_score?.matching_traits) ? profile.yes_score.matching_traits : []
    },
    maybe_score: {
      confidence: profile.maybe_score?.confidence || 0,
      quotes: Array.isArray(profile.maybe_score?.quotes) ? profile.maybe_score.quotes : [],
      matching_traits: Array.isArray(profile.maybe_score?.matching_traits) ? profile.maybe_score.matching_traits : []
    },
    no_score: {
      confidence: profile.no_score?.confidence || 0,
      quotes: Array.isArray(profile.no_score?.quotes) ? profile.no_score.quotes : [],
      matching_traits: Array.isArray(profile.no_score?.matching_traits) ? profile.no_score.matching_traits : []
    },
