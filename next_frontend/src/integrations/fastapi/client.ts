	...
	async getHiredAgents(): Promise<HiredAgent[]> {
		...
		try {
			...
			if (!response.ok) {
				...
				throw new Error(
					`Failed to fetch hired agents: ${response.status} ${errorText}`
				);
			}
	...
	async hireAgent(data: HiredAgentCreate): Promise<HiredAgent> {
		...
		try {
			...
			if (!response.ok) {
				...
				console.error("Error response from server:", errorText);
				throw new Error(
					`Failed to hire agent: ${response.status} ${errorText}`
				);
			}
	...
	async unhireAgentByTemplateId(templateId: string) {
		...
		try {
			...
			const response = await fetch(
				`${API_URL}/hired_agents/${agentToUnhire.id}`,
				{
					method: "DELETE",
					headers,
				}
			...
			if (!response.ok) {
				...
				console.error("Error response from server:", errorText);
				throw new Error(
					`Failed to unhire agent: ${response.status} ${errorText}`
				);
			}
