      ...
      <div className="mx-auto max-w-screen-xl px-4 py-8 sm:py-12 sm:px-6 lg:py-16 lg:px-8">
        ...
        <div className="mt-12 grid grid-cols-1 gap-6 md:grid-cols-2 lg:gap-8 max-w-4xl mx-auto">
          {pricingPlans.map((plan, index) => (
            ...
            >
              {plan.popular && (
                <span className="absolute top-0 right-1/2 transform translate-x-1/2 -translate-y-1/2 rounded-full bg-yellow-400 px-3 py-1 text-xs font-semibold uppercase text-gray-900">
                  Popular
                </span>
              )}
              {plan.discount && (
              ...
              <div className="mt-4">
                {plan.originalPrice && (
                  <p
                    className={`text-lg line-through ${plan.popular ? "text-gray-400" : "text-gray-500"}`}
                  >
                    {plan.originalPrice}
                  </p>
                )}
