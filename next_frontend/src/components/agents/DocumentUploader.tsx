    ...
    <div className="mt-6 pt-6 border-t border-gray-200/50">
      ...
      <div
        ...
        className={`p-6 border-2 border-dashed rounded-lg text-center cursor-pointer transition-colors ${
          isDragActive
            ...
            : (darkMode ? 'border-gray-700 hover:border-blue-500' : 'border-gray-300 hover:border-blue-400')
        }`}
      >
        <input {...getInputProps()} />
        <UploadCloud className={`mx-auto h-10 w-10 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`} />
      ...
      {files.length > 0 && (
        <div className="mt-4 space-y-2">
          <h5 className={`text-sm font-medium ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Ready to upload:</h5>
          {files.map(file => (
            <div
              key={file.name}
            ...
            >
              <div className="flex items-center space-x-2 overflow-hidden">
                <File className={`h-5 w-5 flex-shrink-0 ${darkMode ? 'text-gray-300' : 'text-gray-600'}`} />
                <span className={`truncate ${darkMode ? 'text-gray-200' : 'text-gray-700'}`}>{file.name}</span>
              </div>
              <button onClick={() => removeFile(file.name)} className="p-1 rounded-full hover:bg-gray-500/20">
