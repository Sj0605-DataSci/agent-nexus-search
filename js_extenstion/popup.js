...
async function getJSessionId() {
  ...
  if (!_jsessionCache) {
    console.log("No JSESSIONID found in storage, attempting to retrieve automatically...");
    try {
      // Ask background script to retrieve JSESSIONID
      const response = await new Promise(resolve => {
        chrome.runtime.sendMessage({ type: "linkedin-get-jsessionid" }, resolve);
      });
      ...
      if (response?.ok && response.jsessionId) {
        ...
        console.log("Failed to retrieve JSESSIONID automatically", response?.error || "Unknown error");
      }
    } catch (error) {
    ...
    chrome.runtime.sendMessage({ type: "linkedin-signout" }, (response) => {
      ...
      setTimeout(() => {
        ...
        if (response && response.success) {
          ...
          console.error('Error during sign out:', response?.error || 'Unknown error');
        }
      }, 300);
    });
  } catch (error) {
...
async function autoSyncIfNeeded() {
  try {
    const { lastSyncTime, LINKEDIN_CONNECTIONS } = await chrome.storage.local.get(['lastSyncTime', 'LINKEDIN_CONNECTIONS']);
    const oneHourAgo = new Date();
    oneHourAgo.setHours(oneHourAgo.getHours() - 1);
    
    const shouldSync = !lastSyncTime || new Date(lastSyncTime) < oneHourAgo;
    const hasNoConnections = !LINKEDIN_CONNECTIONS || LINKEDIN_CONNECTIONS.length === 0;
    ...
    if (shouldSync || hasNoConnections) {
      ...
      const { CONNECTION_TOTAL_COUNT } = await chrome.storage.local.get('CONNECTION_TOTAL_COUNT');
...
async function initialize() {
  ...
  if (stored && (stored.profile || stored.tokens?.profile)) {
    ...
    const { LINKEDIN_CONNECTIONS } = await chrome.storage.local.get('LINKEDIN_CONNECTIONS');
    ...
    if (!jsessionId) {
      ...
      chrome.runtime.sendMessage({ type: "linkedin-get-jsessionid" }, async (res) => {
...
function displayConnections(connections) {
  ...
  connectionsList.innerHTML = connections.map(conn => `
    <a href="${conn.profileUrl}" target="_blank" class="connection-item">
      <img 
        src="${conn.profileImg || 'https://static.licdn.com/sc/h/1c5u578iilxfi4m4dvc4q810q'}" 
...
linkedinGetDataApiBtn.addEventListener("click", // Main function to fetch and sync connections
  ...
  try {
      ...
      fetch(
        "https://www.linkedin.com/voyager/api/relationships/dash/connections?decorationId=com.linkedin.voyager.dash.deco.web.mynetwork.ConnectionListWithProfile-16&count=40&q=search&sortType=RECENTLY_ADDED",
        {
          headers: {
            ...
            "x-li-track": '{"clientVersion":"1.13.36912","mpVersion":"1.13.36912","osName":"web","timezoneOffset":5.5,"timezone":"Asia/Calcutta","deviceFormFactor":"DESKTOP","mpName":"voyager-web","displayDensity":2,"displayWidth":3024,"displayHeight":1964}',
          },
        }
      ),
    ...
    connectionsList.innerHTML = `
      <div class="error" style="padding: 16px; text-align: center; color: #d32f2f;">
        <p style="margin: 0 0 12px;">❌ ${err.message || 'Failed to sync connections'}</p>
...
async function getStoredConnectionCount() {
  const result = await chrome.storage.local.get(['CONNECTION_TOTAL_COUNT', 'lastConnectionCountUpdate']);
  if (result.CONNECTION_TOTAL_COUNT) {
    // If we have a stored count that's less than 24 hours old, use it
    const lastUpdate = result.lastConnectionCountUpdate ? new Date(result.lastConnectionCountUpdate) : null;
